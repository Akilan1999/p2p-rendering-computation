...
*** Test rust module
Tests the p2prc go program by calling the test rust function which is
statically linked to the binary only if the module is enabled. There is a tutorial
on how to set this [[#RustTutorialModule][up]].
#+begin_example
