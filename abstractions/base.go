...
func MapPort(port string, domainName string, serverAddress string, udp bool) (response *client.ResponseMAPPort, err error) {
	response, err = client.MAPPort(port, domainName, serverAddress, udp)
	return
}
